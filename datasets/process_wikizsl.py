...
def fewrel_preprocess(train_src_path, train_dst_path, val_src_path, val_dst_path, test_src_path, kr):
    ...
    pid2name = json.load(open('./wikizsl/pid2name.json'))
    # 组织数据
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_way', type=int, default=60)
    parser.add_argument('--way', type=int, default=40)
    ...
    for w in [20, 30, 35, 40]:
        ...
        set_seed(args.seed)
        print(args)
        train_src, train_dst, val_src, val_dst, test_src = './wikizsl/train.jsonl', f'./wikizsl/{args.way}-way-{args.ks}-shot/', './wikizsl/dev.jsonl', f'./wikizsl/{args.way}-way-{args.ks}-shot/', './wikizsl/test.jsonl'
